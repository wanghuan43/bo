...
class Contract extends BoModel
{
    ...
    public function getList($search, $limit,$aType=false,$trashed=2)
    {
        ...
        if ($member->m_isAdmin == "2") {
            ...
            $this->where("(ct.c_id IN (SELECT ci_otid FROM kj_circulation WHERE ci_mid = ".$member->m_id." and ci_type = 'contract') OR ct.c_mid = ".$member->m_id.")")
                ->group('ct.c_id');
        }
    ...
    public function doImport($dataset,&$result,$forceUpdate)
    {
        ...
        foreach ($dataset as $key => $data) {
            ...
            if (!!$company) {
                ...
                $result['failed'][] = array_merge($data,['error'=>'对方公司未找到']);
                unset($dataset[$key]);
                continue;
            }
            ...
            if(empty($forceUpdate)){
                $res = $this->where('c_no','=',$data['c_no'])->where('c_type','=',$data['c_type'])
                    ->where('c_coid','=',$data['c_coid'])->where('c_date','=',$data['c_date'])->find();
                if($res){
                    CustomUtils::writeImportLog('Contract is already exist - '.serialize($data),strtolower($this->name));
                    $result['failed'][] = array_merge($data,['error'=>'合同已存在']);
                    unset($dataset[$key]);
                    continue;
                }
            }
